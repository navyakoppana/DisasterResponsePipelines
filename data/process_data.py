...
def clean_data(df):
    """
    Function to clean data in pandas dataframe

    INPUT
    df - pandas dataframe

    OUTPUT
    Dataframe - returns pandas dataframe that doesn't contain duplicate data
    """
    # drop duplicates
...
def save_data(df, database_filename):
    """
    Funtion to save df into sqlite
    ...
    df.to_sql('Disaster_Response_Pipeline', engine,if_exists='replace', index=False)
...
def main():
     ...
     """
    Function to load, clean and save data in sqlite database
    """
     if len(sys.argv) == 4:
